...
async function getRecord(productId: number) {
    try {
        ...
        const coll = cp
            ...
            .collection(dbConfig.ProductsCollection);
    ...
    try {
        ...
        if (record) {
            ...
            await coll.updateOne(
                { productId: filtered_data["productId"] },
                {
                    $push: {
            ...
            if (response.ok) {
                ...
                for (const script of scripts) {
                    ...
                    if (str_script.startsWith("<script>window.__myx =")) {
                        const json_data = await JSON.parse(
                            str_script.match("{.*}")?.at(0)!
                        );
                        const products_data: Array<any> =
                            json_data["searchData"]["results"]["products"];
                        //products_data.forEach((product) => {
                        for (var product of products_data) {
                            product["images"].map(
                                (img: any, idx: number, arr: Array<any>) => {
                                    arr[idx] = img["src"];
                                }
                            );
                            const filtered_data = {
...
async function fetchBrand(
    ...
    let page_number = 1;
    let hasNext = await fetchBrandPage(
        fin_url,
        brand_name,
        category,
        page_number
...
export default async function scrapeNewIds() {
    try {
        const process_limit = pLimit(
            Number.parseInt(process.env.NUM_PROCESS_FETCH!)
        );

        const cprm = await clientPromise;
        const collection = cprm
            .db(dbConfig.DataBase)
            .collection(dbConfig.ConfigCollection);
        const myntraConfig: any = (
            await collection
                .find({ ScrapeSource: { $regex: new RegExp("Myntra", "i") } })
                .toArray()
        ).at(0);
